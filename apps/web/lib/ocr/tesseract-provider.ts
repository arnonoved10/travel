...
// קאש ייעודי (במקום ברירת-המחדל — תיקיית-העבודה הנוכחית) כדי שקובצי נתוני-השפה
// שמורדים לא יתפזרו בשורש הפרויקט. מוחרג מ-git (ר' .gitignore) — בדיוק כמו
// node_modules/.next, לא תוכן-מקור.
const CACHE_PATH = path.join(process.cwd(), ".tesseract-cache");
