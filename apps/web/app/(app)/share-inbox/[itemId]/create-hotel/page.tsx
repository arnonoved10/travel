...
import { mapExtractedFieldsToHotelPrefill, type HotelPrefill } from "@/lib/ocr/map-extracted-fields";
import { CreateHotelFromScanForm } from "./create-hotel-from-scan-form";

export const dynamic = "force-dynamic";

export default async function CreateHotelFromSharedItemPage({ params }: { params: Promise<{ itemId: string }> }) {
  ...
  try {
    ...
    if (result.ok) {
      prefill = mapExtractedFieldsToHotelPrefill(result.fields);
    } else {
  ...
  return (
    <div>
      <h1 style={{ marginTop: 0 }}>יצירת הזמנת מלון מהתמונה</h1>
      ...
      <div style={{ display: "flex", gap: "1.5rem", flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <CreateHotelFromScanForm itemId={item.id} trips={trips.map((t) => ({ id: t.id, name: t.name }))} prefill={prefill} />
      </div>
    </div>
  );
}
