...
import { Card, IconSlot, ScreenHeader, ScreenShell, BottomNav, COLOR } from "../shared";
      ...
      {tab === "overview" ? (
        <>
          ...
          <Card style={{ border: `1px solid ${COLOR.purple}55`, background: "linear-gradient(160deg, rgba(102,66,185,0.22), #0a1830)" }}>
            ...
            <button type="button" onClick={() => openDialog({ type: "addMoney", currency: localCurrency.currencyCode })} style={{ display: "block", width: "100%", textAlign: "start", background: "none", border: "none", padding: 0, cursor: "pointer", marginBottom: "10px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "6px" }}>
              <button type="button" onClick={() => openDialog({ type: "addMoney", currency: localCurrency.currencyCode })} style={miniActionBtn(COLOR.success)}>
                הוספת כסף
              </button>
              ...
              <button type="button" onClick={() => openDialog({ type: "convert" })} style={miniActionBtn(COLOR.purple)}>
                המרה
              </button>
              <button type="button" onClick={() => openDialog({ type: "history", currency: localCurrency.currencyCode })} style={miniActionBtn(COLOR.textSecondary)}>
                היסטוריה
              </button>
            </div>
          </Card>
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(3, Math.max(1, secondaryBalances.length))}, 1fr)`, gap: "6px" }}>
              {secondaryBalances.length === 0 ? (
                ...
                secondaryBalances.map((b) => {
                  ...
                  return (
                    <button key={b.code} type="button" onClick={() => openDialog({ type: "currencyDetail", currency: b.code })} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "3px", padding: "9px 4px", borderRadius: "12px", background: COLOR.cardBg, border: `1px solid ${COLOR.cardBorder}`, cursor: "pointer" }}>
          ...
          {/* פעולות מהירות */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "6px" }}>
            <QuickAction label="הוצאה" onClick={() => openDialog({ type: "addExpense" })} />
            <QuickAction label="כסף" onClick={() => openDialog({ type: "addMoney", currency: localCurrency.currencyCode })} />
            <QuickAction label="המרה" onClick={() => openDialog({ type: "convert" })} />
      ...
      {tab === "activity" ? (
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button type="button" onClick={() => openDialog({ type: "addExpense" })} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 13px", borderRadius: "999px", background: "rgba(138,90,223,0.18)", border: `1px solid ${COLOR.purple}55`, color: "#c9b3ff", fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>
              + הוספת הוצאה
            </button>
          </div>
